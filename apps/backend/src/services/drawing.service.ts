...
export async function createDrawing(
  ...
  await recordAudit({
    ...
    entityType: 'drawing',
    entityId: d.id,
    newValue: { drawingNo: d.drawingNo, title: d.title },
    ipAddress: ip,
  });
  return d;
}
...
export async function updateDrawing(
  ...
  const updated = await prisma.drawing.update({
    ...
    include: { project: { select: { id: true, name: true } }, currentVersion: true },
  });
  await recordAudit({
    companyId,
    userId,
...
export async function addVersion(
  ...
  const version = await prisma.$transaction(async (tx) => {
    const v = await tx.drawingVersion.create({
      data: {
        drawingId: id,
  ...
  await recordAudit({
    ...
    entityId: version.id,
    newValue: { versionLabel: version.versionLabel },
    ipAddress: ip,
  });
  return version;
}
...
export async function replacePins(
  ...
  userId: string,
  id: string,
  input: ReplaceDrawingPinsInput,
  ip?: string,
) {
  await getDrawing(companyId, id);
  const updated = await prisma.drawing.update({
    where: { id },
    data: { pins: input.pins as Prisma.InputJsonValue },
    include: {
      currentVersion: true,
  ...
  await recordAudit({
    ...
    userId,
    action: 'UPDATE',
    entityType: 'drawing_pins',
    entityId: id,
    newValue: { pinCount: input.pins.length },
    ipAddress: ip,
  });
  return updated;
}
