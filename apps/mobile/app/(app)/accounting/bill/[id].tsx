...
export default function BillDetailScreen() {
  ...
  const metaCard = <BillMetaCard bill={bill} onOpenWorkOrder={() => openWorkOrder(router, bill)} />;
...
function BillMetaCard({ bill, onOpenWorkOrder }: { bill: Bill; onOpenWorkOrder: () => void }) {
  return (
    <Card>
...
function BillActions({
  ...
  return (
    <View className="gap-3">
      ...
      {canApprove && bill.status === 'PENDING' && (
        <Button label="Approve for payment" onPress={onApprove} loading={approvePending} />
      )}
...
function BillStickyActions({
  ...
  if (canApprove && bill.status === 'PENDING') {
    return <Button label="Approve bill" onPress={onApprove} loading={approvePending} />;
  }
